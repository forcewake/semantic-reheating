...
CI renders the generated assets twice into temporary directories and requires within-environment byte stability without replacing the packaged preview. The checked-in PNG and release readback are the publication bytes. Cross-environment byte equality is not claimed because native renderer packages can encode equivalent raster output differently across supported runner environments.
