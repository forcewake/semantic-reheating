...
def detect_repeated_error(trace: Any, policy: Any) -> dict[str, Any]:
    ...
    first_errors: dict[tuple[str, tuple[str, str] | tuple[str, str, str] | None], str] = {}
    for event in window:
        if event.kind is TraceKind.TOOL_CALL:
            identity = _identity(event)
            context = identity if identity is not None else ("unidentified_call", event.event_id, "")
            continue
        ...
        if first is not None:
            return _finding("repeated_error", window, parsed_policy, [first, event.event_id], True)
        first_errors[key] = event.event_id
    return _finding("repeated_error", window, parsed_policy, [window[-1].event_id], False)
